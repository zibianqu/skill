...
class CreateGoodsTable extends Migration {
	...
	public function up()
	{
		Schema::create('goods', function(Blueprint $table)
		{
			...
			$table->string('goods_name', 100)->default('')->index('goods_name_i')->comment('商品名称');
			$table->string('shop_id', 50)->default('')->index('shop_id_i')->comment('商铺id');
			$table->string('goods_no', 50)->default('')->unique('goods_no_i')->comment('商品编号');
			$table->integer('goods_type')->default(0)->index('goods_type_i')->comment('商品类型');
